...
import { TokenExpiredError, TokenInvalidError } from '../src/errors.js';
...
describe('Crypto utilities', () => {
	...
	describe('verifySignedToken', () => {
		...
		it('should throw error for token with wrong number of parts', async () => {
			await expect(verifySignedToken('part1.part2.part3', 'secret')).rejects.toThrow(
				'Signed token must have 2 parts',
			);
		});
