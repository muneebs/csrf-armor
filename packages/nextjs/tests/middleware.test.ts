...
describe("CSRF Middleware", () => {
  ...
  it("should generate proper tokens for signed-double-submit strategy", async () => {
    ...
    const serverCookieToken = result.response.cookies.get("csrf-token-server")?.value;
  ...
  it("should validate signed-double-submit POST request", async () => {
    ...
    const clientCookieToken = getResult.response.cookies.get("csrf-token")?.value;
    const serverCookieToken = getResult.response.cookies.get("csrf-token-server")?.value;
  ...
  it("should reject signed-double-submit with mismatched client and server cookies", async () => {
    ...
    const clientCookieToken = getResult1.response.cookies.get("csrf-token")?.value;
    const serverCookieToken = getResult2.response.cookies.get("csrf-token-server")?.value; // Different!
